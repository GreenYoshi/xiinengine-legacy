...
$dbvars = $this->database->query("DESCRIBE XE_NCategories") or die($this->database->error);
...
while ($perm = $dbvars->fetch_assoc())
{
	...
	switch ($perm['Field'])
	{
		case 'PermAccessName':
			...
			{
				$okdata = false;
				$error = "Category Name field cannot be empty!";
			}
			else
			{
				if ($firstentry)
...
else
{
	...
	{
		$this->database->query("INSERT INTO XE_NCategories (".$insertvarset.") VALUES(".$insertvalset.")") or die (mysql_error());
