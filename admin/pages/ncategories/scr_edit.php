...
$permquery = "SELECT * FROM XE_NCategories WHERE NCatID = '".$requestID."'";
...
foreach ($perm as $id => $info)
{
	...
	switch ($id)
	{
		case 'NCatName': //Regenerate the pretty url if the alias has changed
			if (empty($info))
			{
				$okdata = false;
			...
			break;
		case 'NCatID': //the 'do nothing'/uneditable fields
			break;
		default:
			$escaped = $this->database->real_escape_string(${$id});
			if ($firstentry)
...
else
{
	if ($updateset != "")
	{
		...
		$this->database->query("UPDATE XE_NCategories SET ".$updateset." WHERE NCatID = '".$perm['NCatID']."'") or die($this->database->error);
	}
